...
// WithRequestID sets the RequestID in the LogContext
func (lc *LogContext) WithRequestID(requestID string) *LogContext {
	lc.RequestID = requestID
	return lc
}

// WithAccountID sets the AccountID in the LogContext
func (lc *LogContext) WithAccountID(accountID string) *LogContext {
	lc.AccountID = accountID
	return lc
}

// WithTraceID sets the TraceID in the LogContext
func (lc *LogContext) WithTraceID(traceID string) *LogContext {
	lc.TraceID = traceID
	return lc
}

// WithSpanID sets the SpanID in the LogContext
func (lc *LogContext) WithSpanID(spanID string) *LogContext {
	lc.SpanID = spanID
	return lc
}

// WithOperation sets the Operation in the LogContext
func (lc *LogContext) WithOperation(operation string) *LogContext {
	lc.Operation = operation
	return lc
}

// WithComponent sets the Component in the LogContext
...
func (lc *LogContext) ToFields() []zap.Field {
	fields := make([]zap.Field, 0)
	if lc.RequestID != "" {
		fields = append(fields, zap.String("request_id", lc.RequestID))
	}

	if lc.AccountID != "" {
		fields = append(fields, zap.String("account_id", lc.AccountID))
	}
	...
	if lc.Operation != "" {
		fields = append(fields, zap.String("operation", lc.Operation))
	}

	if lc.Component != "" {
		fields = append(fields, zap.String("component", lc.Component))
	}
...
func FileConfigFromGlobal() *FileConfig {
	return &FileConfig{
		SavePath:   configs.GlobalConfig.Log.SavePath,
		FileName:   configs.GlobalConfig.Log.FileName,
		MaxSize:    configs.GlobalConfig.Log.MaxSize,
		MaxAge:     configs.GlobalConfig.Log.MaxAge,
		LocalTime:  configs.GlobalConfig.Log.LocalTime,
		Compress:   configs.GlobalConfig.Log.Compress,
		MaxBackups: 1,
	}
}
...
func (l *AppLogger) createLumberjackHook() func(zapcore.Entry) error {
	...
	return func(e zapcore.Entry) error {
		...
		lum := &lumberjack.Logger{
			Filename:   filepath.Join(rootPath, fc.SavePath, fc.FileName),
			MaxSize:    fc.MaxSize,
			MaxAge:     fc.MaxAge,
...
func Init() {
	...
	if configs.GlobalConfig.Log.Level != "" {
		// Use configured log level if available
		logLevel = ParseLogLevel(configs.GlobalConfig.Log.Level)
	} else if !configs.GlobalConfig.Env.IsProduction() {
	...
	opts = append(opts, WithConsole(configs.GlobalConfig.Log.EnableConsole))

	// Configure colorized output (defaults to enabled in non-production)
	opts = append(opts, WithColor(configs.GlobalConfig.Log.EnableColor))

	// Configure caller information (defaults to true if not specified)
	enableCaller := configs.GlobalConfig.Log.EnableCaller
